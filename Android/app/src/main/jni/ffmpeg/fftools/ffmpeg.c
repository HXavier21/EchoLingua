...
static void update_video_stats(OutputStream *ost, const AVPacket *pkt, int write_vstats) {
    ...
    if (vstats_version <= 1) {
        ...
        fprintf(vstats_file, "out= %2d st= %2d frame= %5"PRId64" q= %2.1f ", ost->file_index,
...
static void do_video_out(OutputFile *of,
    ...
    if (!ost->filters_script &&
        ...
        duration = lrintf(
                next_picture->duration * av_q2d(ist->st->time_base) / av_q2d(enc->time_base));
    }
...
void android_log_callback(void *ptr, int level, const char *fmt, va_list vl) {
    ...
    else android_log_level = ANDROID_LOG_DEBUG;
